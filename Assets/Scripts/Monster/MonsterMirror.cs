...
{
    //   기본 상태값
    private Vector3 _startPosition;     // 거울사제2(몬스터)의 시작 위치
    [SerializeField] 
    private float monsterHp = 20f;      // 거울사제2(몬스터)의 체력
    private bool _isDamaged = false;        // 거울사제2(몬스터)의 피격확인용 변수
    [SerializeField] 
    private float attackDistance = 2f;      // 거울사제2(몬스터)의 공격범위
    [SerializeField]
    private float moveSpeed = 8f;       // 거울사제2(몬스터)의 이동속도
    [SerializeField] 
    private float attackDelay = 2f;     // 거울사제2(몬스터)의 공격 딜레이
    [SerializeField] 
    private float attackPower = 5f;     // 거울사제2(몬스터)의 공격력
    private float _currentTime = 0f;    // 거울사제2(몬스터)의 현재 공격시간 currentTime이 Delay보다 커진다면 공격 진행
    
    private Transform _player;          // 플레이어의 위치 값 받아오는 용도
    private Animator _animator;         // 거울사제2(몬스터)의 애니메이터
    private NavMeshAgent _navMeshA;     // 거울사제2(몬스터)의 네비매쉬매니저
    
    private Vector3 _randomPosition;
    private Vector3 _distance;
    

    MonsterState m_State;               // 거울사제2(몬스터)의 현재 상태

    [SerializeField] private bool isMovingMonster = true;               // 거울사제2(몬스터)가 움직일지 여부
    [SerializeField] private List<Vector3> moveDirectionList;           // 거울사제2(몬스터)의 탐색 경로 지정리스트
    [SerializeField] private List<float> moveDirectionDelayList;        // 거울사제2(몬스터)각 경로에서 몇초 동안 멈출지 지정리스트
    private bool _isWait = false;                                       // 거울사제2(몬스터) 지금 대기중인지 확인용 변수
    private int _moveDirectionIndex = 0;                                // 거울사제2(몬스터)현재 경로 인덱스
    [SerializeField] private bool DebugMode = false;                    // 거울사제2(몬스터)의 탐색범위 가시화 할지 여부
    [Range(0f, 360f)] [SerializeField] private float ViewAngle = 0f;    // 거울사제2(몬스터)의 탐색범위(시야각)
    [SerializeField] private float ViewRadius = 2f;                     // 거울사제2(몬스터)의 탐색범위(반지름)
    [SerializeField] private LayerMask TargetMask;                      // 거울사제2(몬스터)의 탐색대상 레이어
    [SerializeField] LayerMask ObstacleMask;                            // 거울사제2(몬스터)의 탐색대상 레이어2
    ...
    void Update()
    {
        Mirror.SetActive(!isMirrorAttacked); // 거울 부수기 공격을 하면 거울 오브젝트가 안보이도록
    ...
    IEnumerator WaitIdle(int index) //      거울사제2(몬스터)가 각 루트끝에 몇 초 동안 대기하고 다음 루트를 지정하는 함수
    {
        _isWait = true; //  기다림 시작
    ...
    public void Move()  //  거울사제2(몬스터)추격
    {
        ...
        if (Vector3.Distance(_player.position, transform.position) > SearchDistance) // 플레이어와 거울사제2(몬스터)의 거리가 탐색범위 보다 길다면
        {
        ...
        {   // 만약 플레이어와 거울사제2(몬스터)의 거리가 공격 범위보다 가깝다면 
            m_State = MonsterState.Attack;// 거울사제2(몬스터)의 상태를 Attak으로 변경
    ...
    public void HitedMonster(int hitPower)  //  거울사제2(몬스터)의 체력을 뺴는 함수
    {
        ...
        {
            monsterHp -= hitPower;  // 거울사제2(몬스터)의 체력을 뺌
            m_State = MonsterState.Damaged;// 거울사제2(몬스터)의 상태를 Damaged 변경
